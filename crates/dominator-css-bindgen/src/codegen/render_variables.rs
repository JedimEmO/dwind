...
pub fn render_variable_definitions(module_name: &str, variable_names: Vec<String>) -> String {
    let variable_names = variable_names.into_iter().filter_map(|name| {
        if !name.starts_with("--") {
            return None;
        }
...
fn render_struct_of_variable_names(module_name: &str, variable_names: Vec<String>) -> TokenStream {
    let rustified_variable_names = variable_names.iter().filter_map(|name| {
        name.strip_prefix("--").map(|v| v.to_string())
    }).collect::<Vec<String>>();
    ...
    let raw_string_exprs = variable_names.iter().zip(rustified_variable_names.iter()).map(|(raw_name, struct_field_name)| {
        let field_ident = Ident::new(css_ident_to_rust(struct_field_name).as_str(), Span::call_site());
    ...
    let struct_ident = Ident::new(format!("{}CssVariables", module_name).to_camel().as_str(), Span::call_site());
