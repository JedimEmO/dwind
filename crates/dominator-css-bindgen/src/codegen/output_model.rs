    ...
    for file in parsed_files {
        for block in file.blocks {
            for selector in block.selector {
                match selector {
                    ParsedSelector::SingleClass { class_name, pseudo_classes } => {
                        let out_class = out_classes.entry(class_name.clone()).or_insert(OutputClass {
                            identity: class_name,
                            main_class_body: vec![],
                            pseudo_classes: vec![],
                        });
                        ...
                        if pseudo_classes.is_empty() {
                            ...
                            out_class.pseudo_classes.push(OutputPseudoClass { selector: pseudo_classes, body: block.data.clone() });
                        }
                    }
